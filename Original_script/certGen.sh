...
STATE="77"
LOCALITY="Zelenograd"
ORGANIZATION_NAME="TermoCube Ltd."
root_ca_password="1234"
key_bits_length="4096"
days_till_expire=3650
ca_chain_prefix="termocube.chain.ca"
intermediate_ca_dir="."
...
root_ca_prefix="termocube.root.ca"
intermediate_ca_prefix="termocube.intermediate"
...
function warn_certs_not_for_production()
{
    tput smso
    tput setaf 3
    echo "Настоящий скрипт предназначен для создания сертификатов для системы IoT при взаимодействиии с Azure IoT Hub."
    echo "По умолчанию в скрипте испльзуется пароль ${root_ca_password}."
    echo "Скрипт является доработкой скрипта от Microsoft Corporation и планируется исползовать для генерации ключей и сертификатов."
    echo "По умолчаюнию сертификаты истекают в течнии ${days_till_expire} дней."
    tput sgr0
}

function generate_root_ca()
{
    local common_name="TermoCube Test Project Root Cert"
...
{
    local common_name="TermoCube Test Project Intermediate Cert"
...
if [ "${1}" == "create_root_and_intermediate" ]; then
    ...
    echo "Испльзовние: create_root_and_intermediate                         # Создание новых корневого и промежуточного сертификатов"
    echo "       create_verification_certificate <subjectName>              # Создание проверочного сертификата, подписанного с использованием <subjectName>"
    echo "       create_device_certificate <subjectName>                    # Создание сертификата устройства подписанного корневым сертификатом с <subjectName>"
    echo "       create_device_certificate_from_intermediate <subjectName>  # Создание сертификата устройства подписанного промежуточным сертификатом с <subjectName>"
    echo "       create_edge_device_certificate <subjectName>               # Создание сертификата граничного устройства подписанного с <subjectName>"
    exit 1
fi
